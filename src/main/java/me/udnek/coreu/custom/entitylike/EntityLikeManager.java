...
@NullMarked public abstract class EntityLikeManager<
        Real,
    ...
    protected abstract boolean equals(Real r1, Real r2);

    public List<Entity> getAllLoaded(){
        return new ArrayList<>(loaded);
    }

    public @Nullable Entity getTicking(Real real){
    ...
    public Entity getTickingOrException(Real real){
        Entity ticking = getTicking(real);
        Preconditions.checkArgument(ticking != null, "Ticking not fount: " + real);
        return ticking;
    }

    public boolean isTickingLoaded(Real real){
        return getTicking(real) != null;
    }

    public void loadAny(Type type, Real real){
        type.load(real);
    ...
    public void unloadAny(Type type, Real real){
        if (type instanceof EntityLikeTickingType<?, ?>){
            Entity ticking = getTicking(real);
    ...
    public void unloadTicking(Entity entity){
